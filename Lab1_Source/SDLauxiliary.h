...
void fill_screen(glm::vec3 color);
...
// Starfield
void Draw_Starfield();
void Draw_Starfield(std::vector<glm::vec3>& stars);
void Create_Random_Starfield(std::vector<glm::vec3>& stars);
void Update_motion(std::vector<glm::vec3>& stars);
	...
	while( SDL_PollEvent(&e) )
	{
		...
		if( e.type == SDL_KEYDOWN )
			if( e.key.keysym.sym == SDLK_ESCAPE)
				return false;
		// MAC exit: cmd + q 
		if ( (KMOD_LMETA & e.key.keysym.mod) && e.key.keysym.sym == SDLK_q) 
			return false;
	}
	return true;
}
